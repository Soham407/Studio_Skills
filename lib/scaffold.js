...
function resolveGithubMode(options) {
  if (options.github === false) return 'skip'
  const mode = options.github ?? 'private'
...
async function resolveLocalModelMode(config) {
  ...
  const useLocalModel = cancelIfNeeded(await confirm({
    message: 'Use local model guidance for routine tasks in this project?',
    initialValue: true,
  }))
  return useLocalModel ? 'prefer' : 'skip'
}
...
export async function scaffold(name, options = {}) {
  ...
  const githubMode = resolveGithubMode(options)
  ...
  await setupGithubRepo(projectDir, name, githubMode)
  ...
  await writeAgentDocs(projectDir, name, type, localModelMode, localModelName, localModelRuntime)
  await finalize(projectDir, name, type, githubMode)
}
