...
function claudeMd(projectName, type, ollamaMode, ollamaModel) {
  const ollamaText = ollamaMode === 'skip'
    ? 'Use cloud Claude for all coding and architectural work. Do not assume local Ollama is available.'
    : `Prefer local Ollama (${ollamaModel}) for routine tasks such as file edits, simple refactors, lookups, and formatting. Use cloud Claude for complex architecture, multi-file refactors, domain modeling, and PRD synthesis.`
...
This project ships with studio skills at \`.claude/skills/\`:
...
async function validateEnvironment() {
  ...
  for (const command of ['pnpm', 'gh', 'node']) {
  ...
  if (!(await commandExists('orb'))) {
    console.log(chalk.yellow('[kickstart] orb not found; OrbStack is optional but recommended'))
  }
...
async function setupGithubRepo(projectDir, projectName) {
  logStep('Step 3/8: Creating private GitHub repository')
  ...
  try {
    await execa('gh', ['repo', 'create', projectName, '--private', '--source=.', '--remote=origin', '--push'], { cwd: projectDir, stdio: 'inherit' })
  } catch {
    ...
    if (alternate && alternate !== 'skip') {
      await execa('gh', ['repo', 'create', alternate, '--private', '--source=.', '--remote=origin', '--push'], { cwd: projectDir, stdio: 'inherit' }).catch(() => {
...
async function writeClaudeMd(projectDir, projectName, type, ollamaMode, ollamaModel) {
  logStep('Step 7/8: Writing CLAUDE.md')
  await writeFile(join(projectDir, 'CLAUDE.md'), claudeMd(projectName, type, ollamaMode, ollamaModel))
}

async function finalize(projectDir, projectName, type) {
...
async function resolveOllamaMode(config) {
  if (config.ollama !== 'ask') return config.ollama

  const useOllama = cancelIfNeeded(await confirm({
    message: 'Use local Ollama guidance for routine tasks in this project?',
    initialValue: true,
  }))
  return useOllama ? 'prefer' : 'skip'
}

export async function scaffold(name, options = {}) {
  const type = resolveType(options)
  if (!type || !name) fail('Usage: kickstart --web|--mobile|--universal <project-name>')
  if (!/^[a-z0-9][a-z0-9-]*$/.test(name)) fail(`Project name must be lowercase alphanumeric with hyphens: ${name}`)
  if (existsSync(name)) fail(`Directory already exists: ${name}`)

  const config = loadConfig()
  const repoUrl = getSkillsRepo(options.skills)
  const ollamaMode = await resolveOllamaMode(config)

  console.log(chalk.cyan('\nkickstart'))
  console.log(`Bootstrapping ${name} (${type})`)

  await validateEnvironment()
  const projectDir = await createProject(type, name)
  await setupGithubRepo(projectDir, name)
  ...
  await writeClaudeMd(projectDir, name, type, ollamaMode, config.ollamaModel)
  await finalize(projectDir, name, type)
}
