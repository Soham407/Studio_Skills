...
program
  ...
  .option('--github <mode>', 'GitHub repo mode: private, public, or skip', 'private')
...
skills
  ...
  .option('--agent <agent>', `Agent target: ${Object.keys(AGENT_SKILL_TARGETS).join(', ')}`, 'claude')
  .action(async (skillName, options) => {
    const repoUrl = getSkillsRepo(options.skills)
    const skill = await installSkill(options.target, repoUrl, skillName, options.agent)
